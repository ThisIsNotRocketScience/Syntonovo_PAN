...
static int usbmidi_msglen(const midicmd_t cmd)
{
	...
	case 0xD: // channel pressure
		return 2; // 2 uint8_ts
	case 0x3: // 3-uint8_t system common
	case 0x4: // 3-uint8_t sysex start/continue
	case 0x7: // 3-uint8_t sysex end
	case 0x8: // note off
	case 0x9: // note on
...
{
	cmd->header = 0x4;
	cmd->b1 = state->midiin_uint8_ts[0];
	cmd->b2 = state->midiin_uint8_ts[1];
	cmd->b3 = state->midiin_uint8_ts[2];
}
...
static void midiin_pack_cmd(midicmd_t* cmd, midiin_state_t* state)
{
	if (state->midiin_status < 0xF0) {
		...
		if (state->midiin_uint8_ts[0] == 0xF0) {
			cmd->header = 0x4;
		}
		else {
			switch (state->midiin_count) {
	...
	cmd->b2 = state->midiin_uint8_ts[0];
	cmd->b3 = state->midiin_uint8_ts[1];
}

int midiin_process(midicmd_t* cmd, uint8_t b, midiin_state_t* state)
{
	if (b == 0xf4 || b == 0xf5) {
		// ignore internal sync uint8_ts
		return 0;
	}

	if (b >= 0xF8) {
		// single uint8_t system realtime messages
		cmd->header = 0xF;
		cmd->b1 = b;
		cmd->b2 = 0;
		cmd->b3 = 0;
	...
	if (b == 0xF7) {
		// SysEx End
		state->midiin_uint8_ts[state->midiin_count] = b;
		state->midiin_count++;
...
int midiout_checkstatus(uint8_t status, midiout_state_t* state)
{
	// allow non-status uint8_ts
	if ((status & 0x80) == 0) {
		return 1;
	}

	// allow all real-time status uint8_ts
	if ((status & 0xF0) == 0xF0) {
		if (status <= 0xF7) {
	...
	// filter rest of status uint8_ts if status did not change

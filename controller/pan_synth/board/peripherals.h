/***********************************************************************************************************************
 * This file was generated by the MCUXpresso Config Tools. Any manual edits made to this file
 * will be overwritten if the respective MCUXpresso Config Tools is used to update this file.
 **********************************************************************************************************************/

#ifndef _PERIPHERALS_H_
#define _PERIPHERALS_H_

/***********************************************************************************************************************
 * Included files
 **********************************************************************************************************************/
#include "fsl_common.h"
#include "fsl_spi.h"
#include "fsl_clock.h"
#include "fsl_sctimer.h"
#include "fsl_gpio.h"
#include "fsl_reset.h"
#include "fsl_usart.h"

#if defined(__cplusplus)
extern "C" {
#endif /* __cplusplus*/

/***********************************************************************************************************************
 * Definitions
 **********************************************************************************************************************/
/* Definitions for BOARD_InitPeripherals functional group */
/* BOARD_InitPeripherals defines for FLEXCOMM2 */
/* Definition of peripheral ID */
#define SPI_2_PERIPHERAL ((SPI_Type *)FLEXCOMM2)
/* Definition of the clock source */
#define SPI_2_CLKSRC kCLOCK_Flexcomm2
/* SPI_2 interrupt vector ID (number). */
#define SPI_2_FLEXCOMM_IRQN FLEXCOMM2_IRQn
/* SPI_2 interrupt handler identifier. */
#define SPI_2_FLEXCOMM_IRQHANDLER FLEXCOMM2_IRQHandler
/* BOARD_InitPeripherals defines for FLEXCOMM5 */
/* Definition of peripheral ID */
#define SPI_5_PERIPHERAL ((SPI_Type *)FLEXCOMM5)
/* Definition of the clock source */
#define SPI_5_CLKSRC kCLOCK_Flexcomm5
/* SPI_5 interrupt vector ID (number). */
#define SPI_5_FLEXCOMM_IRQN FLEXCOMM5_IRQn
/* SPI_5 interrupt handler identifier. */
#define SPI_5_FLEXCOMM_IRQHANDLER FLEXCOMM5_IRQHandler
/* BOARD_InitPeripherals defines for FLEXCOMM6 */
/* Definition of peripheral ID */
#define SPI_6_PERIPHERAL ((SPI_Type *)FLEXCOMM6)
/* Definition of the clock source */
#define SPI_6_CLKSRC kCLOCK_Flexcomm6
/* SPI_6 interrupt vector ID (number). */
#define SPI_6_FLEXCOMM_IRQN FLEXCOMM6_IRQn
/* SPI_6 interrupt handler identifier. */
#define SPI_6_FLEXCOMM_IRQHANDLER FLEXCOMM6_IRQHandler
/* BOARD_InitPeripherals defines for FLEXCOMM7 */
/* Definition of peripheral ID */
#define SPI_7_PERIPHERAL ((SPI_Type *)FLEXCOMM7)
/* Definition of the clock source */
#define SPI_7_CLKSRC kCLOCK_Flexcomm7
/* SPI_7 interrupt vector ID (number). */
#define SPI_7_FLEXCOMM_IRQN FLEXCOMM7_IRQn
/* SPI_7 interrupt handler identifier. */
#define SPI_7_FLEXCOMM_IRQHANDLER FLEXCOMM7_IRQHandler
/* BOARD_InitPeripherals defines for FLEXCOMM3 */
/* Definition of peripheral ID */
#define SPI_3_PERIPHERAL ((SPI_Type *)FLEXCOMM3)
/* Definition of the clock source */
#define SPI_3_CLKSRC kCLOCK_Flexcomm3
/* SPI_3 interrupt vector ID (number). */
#define SPI_3_FLEXCOMM_IRQN FLEXCOMM3_IRQn
/* SPI_3 interrupt handler identifier. */
#define SPI_3_FLEXCOMM_IRQHANDLER FLEXCOMM3_IRQHandler
/* BOARD_InitPeripherals defines for SCT0 */
/* Definition of peripheral ID */
#define SCTIMER_1_PERIPHERAL SCT0
/* Definition of clock source frequency */
#define SCTIMER_1_CLOCK_FREQ CLOCK_GetFreq(kCLOCK_BusClk)
/* SCTimer_1 interrupt vector ID (number). */
#define SCTIMER_1_IRQN SCT0_IRQn
/* SCTimer_1 interrupt handler identifier. */
#define SCTIMER_1_IRQHANDLER SCT0_IRQHandler
/* SCTimer output 0 mask */
#define SCTIMER_OUTPUT_0_MASK (1U << kSCTIMER_Out_0)
/* SCTimer output 1 mask */
#define SCTIMER_OUTPUT_1_MASK (1U << kSCTIMER_Out_1)
/* SCTimer output 2 mask */
#define SCTIMER_OUTPUT_2_MASK (1U << kSCTIMER_Out_2)
/* SCTimer output 3 mask */
#define SCTIMER_OUTPUT_3_MASK (1U << kSCTIMER_Out_3)
/* SCTimer output 4 mask */
#define SCTIMER_OUTPUT_4_MASK (1U << kSCTIMER_Out_4)
/* SCTimer output 5 mask */
#define SCTIMER_OUTPUT_5_MASK (1U << kSCTIMER_Out_5)
/* SCTimer output 6 mask */
#define SCTIMER_OUTPUT_6_MASK (1U << kSCTIMER_Out_6)
/* SCTimer output 7 mask */
#define SCTIMER_OUTPUT_7_MASK (1U << kSCTIMER_Out_7)
/* Alias for GPIO peripheral */
#define GPIO_1_GPIO GPIO
/* BOARD_InitPeripherals defines for FLEXCOMM8 */
/* Definition of peripheral ID */
#define SPI_8_PERIPHERAL ((SPI_Type *)FLEXCOMM8)
/* Definition of the clock source */
#define SPI_8_CLKSRC kCLOCK_Flexcomm8
/* SPI_8 interrupt vector ID (number). */
#define SPI_8_FLEXCOMM_IRQN FLEXCOMM8_IRQn
/* SPI_8 interrupt handler identifier. */
#define SPI_8_FLEXCOMM_IRQHANDLER FLEXCOMM8_IRQHandler
/* Definition of peripheral ID */
#define USART_1_PERIPHERAL ((USART_Type *)FLEXCOMM1)
/* Definition of the clock source frequency */
#define USART_1_CLOCK_SOURCE CLOCK_GetFreq(kCLOCK_Flexcomm1)
/* USART_1 interrupt vector ID (number). */
#define USART_1_FLEXCOMM_IRQN FLEXCOMM1_IRQn
/* USART_1 interrupt handler identifier. */
#define USART_1_FLEXCOMM_IRQHANDLER FLEXCOMM1_IRQHandler

/***********************************************************************************************************************
 * Global variables
 **********************************************************************************************************************/
extern const spi_master_config_t SPI_2_config;
extern const spi_master_config_t SPI_5_config;
extern const spi_master_config_t SPI_6_config;
extern const spi_master_config_t SPI_7_config;
extern const spi_master_config_t SPI_3_config;
extern const sctimer_config_t SCTimer_1_initConfig;
extern uint32_t SCTimer_1_event[2];
extern gpio_pin_config_t GPIO_1_config[3];
extern const spi_master_config_t SPI_8_config;
extern const usart_config_t USART_1_config;

/***********************************************************************************************************************
 * Initialization functions
 **********************************************************************************************************************/
void BOARD_InitPeripherals(void);

/***********************************************************************************************************************
 * BOARD_InitBootPeripherals function
 **********************************************************************************************************************/
void BOARD_InitBootPeripherals(void);

#if defined(__cplusplus)
}
#endif

#endif /* _PERIPHERALS_H_ */

...
void spi_sched_init()
{
   ...
   // NVIC_SetPriority(SPI_2_FLEXCOMM_IRQN, 2);
    ...
    NVIC_SetPriority(SPI_4_FLEXCOMM_IRQN, 2);
...
{
	spi_sched_irq(spi_bus[0], &spi_bus_queue[0]);
}

__attribute__( ( section(".data") ) )
WEAK void FLEXCOMM1_IRQHandler(void)
{
	spi_sched_irq(spi_bus[1], &spi_bus_queue[1]);
}
...
{
	spi_sched_irq(spi_bus[4], &spi_bus_queue[4]);
}

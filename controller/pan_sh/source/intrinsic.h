...
__attribute__( ( always_inline ) ) static __INLINE uint32_t umlal32_hi(uint32_t acc, uint32_t a, uint32_t b)
{
	uint32_t tmp_lo = 0;

	__ASM volatile ("umlal %0, %1, %2, %3" : "+r" (tmp_lo), "+r" (acc) : "r" (a), "r" (b) );
	return acc;
}

#if 0
__attribute__( ( always_inline ) ) static __INLINE uint32_t umull32_hi(uint32_t a, uint32_t b)
{
	uint32_t acc = 0;
	uint32_t tmp_lo = 0;

	__ASM volatile ("umull %0, %1, %2, %3" : "+r" (tmp_lo), "+r" (acc) : "r" (a), "r" (b) );
	return acc;
}
#endif
